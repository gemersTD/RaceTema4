...
```{r}
cran3 <- cran[, c("ip_id", "package", "size")]
...
```{r}
top_counts <- by_package %>% summarise(download_count = n()) %>% 
  filter(download_count > quantile(download_count, probs = 0.99))
